...
def img_alexnet_layers(img, batch_size, output_dim, stage, model_weights=None, val_batch_size=32, with_tanh=True):
    deep_param_img = {}
    train_layers = []

    if model_weights is None:
        dir_path = os.path.dirname(os.path.realpath(__file__))
        model_weights = os.path.join(dir_path, "pretrained_model/alexnet_weights.npy")
        # model_weights = os.path.join(dir_path, "pretrained_model/alexnet_weights_trained.npy")
    ...
    with tf.name_scope('preprocess'):
        ...
        def val_fn():
            ...
            def distort(f, x, y): return tf.stack(
                [crop(f(each), x, y) for each in tf.unstack(reshaped_image, val_batch_size)])
