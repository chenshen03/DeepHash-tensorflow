...
def plot_distance(db_feats, db_label, query_feats, query_label, path):
    ...
    commutes = pd.Series(np.random.choice(cosine_32bit[S==1].flatten(), N))
    commutes.plot.hist(grid=True, bins=200, rwidth=1.5, color='#607c8e', alpha=0.7);
    commutes = pd.Series(np.random.choice(cosine_32bit[S==0].flatten(), N))
    commutes.plot.hist(grid=True, bins=200, rwidth=1.5, color='#500c8e', alpha=0.7);

    plt.subplot(122)
    euclidean_32bit = distance.cdist(db_feats, query_feats, metric='euclidean')
    plt.title('euclidean distribution')
    commutes = pd.Series(np.random.choice(euclidean_32bit[S==1].flatten(), N))
    commutes.plot.hist(grid=True, bins=200, rwidth=1.5, color='#607c8e', alpha=0.7);
    commutes = pd.Series(np.random.choice(euclidean_32bit[S==0].flatten(), N))
    commutes.plot.hist(grid=True, bins=200, rwidth=1.5, color='#500c8e', alpha=0.7);
