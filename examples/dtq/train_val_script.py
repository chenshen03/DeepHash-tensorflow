...
parser.add_argument('--n-subspace', default=4, type=int)
parser.add_argument('--n-subcenter', default=256, type=int)
...
train_img = dataset.import_train(data_root, args.img_tr)
model_weights = model.train(train_img, query_img, database_img, args)

args.model_weights = model_weights
maps = model.validation(query_img, database_img, args)
for key in maps:
    print(("{}\t{}".format(key, maps[key])))
