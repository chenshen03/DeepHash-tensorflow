...
def parse_args(argv):
    ...
    algorithm_group.add_argument('--output-dim', type=int, default=300)
    ...
    algorithm_group.add_argument('--margin', type=int, default=0.7)
    # network config
    ...
    network_group.add_argument('--learning-rate', type=float, default=0.02) # 0.02 for DVSQ, 0.002 for DQN
    ...
    _save_dir = f'../snapshot/{config.dataset}_{config.network}_{config.output_dim}bit_dvsq/' + \
            f'{config.prefix}_margin{config.margin}_subspace{config.n_subspace}_subcenter{config.n_subcenter}'
    ...
    dataset_group.add_argument('--wordvec-dict', type=str, default="../../data/cifar10/wordvec.txt")
