...
from loss import cos_margin_multi_label_loss, pq_loss
...
class DVSQ(object):
    def __init__(self, config):
        ...
        with tf.device(self.device):
            ...
            ICM_word_dict = tf.reshape(tf.matmul(tf.reshape(
                        ICM_X_expand - ICM_C_m_expand, [self.code_batch_size * self.subcenter_num, self.output_dim]),
                    tf.transpose(self.wordvec)), [self.code_batch_size, self.subcenter_num, self.n_class])
